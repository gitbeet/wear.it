...
export const PromosBanner = ({
  ...
  const CustomArrow = ({ direction }: { direction: "left" | "right" }) => (
    <button
      ...
      className="grid w-8 shrink-0 grow place-content-center text-white hover:bg-slate-600 active:opacity-50"
      onClick={() =>
        direction === "left"
  ...
  return (
    ...
    <div className="shadow-color relative z-[40] flex w-full  items-center justify-center bg-slate-800 pt-16 text-white shadow-sm">
      <div className="padding-x flex w-[min(100vw,40rem)] items-stretch justify-center gap-2 md:gap-4">
        ...
        >
          {promos.map((banner, i) => (
            <SwiperSlide key={i}>
              <div className="px-1 text-center text-xs font-semibold sm:px-8 md:px-4">
                <p>{banner.text}</p>
                <Link
